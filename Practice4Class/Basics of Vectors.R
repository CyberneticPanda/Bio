...
# Numeric Vectors 
single_numbers <- c(1)
multiple_numbers <- c(1,2,3,4,5,6,7,9,10)
...
# I want th efirst and third element of the states vector 
?subset
str(count)
count
...
# creating locical vectors with "us" functions
is.factor(count)
is.function(states)
is.vector(states)
...
# null values in R are represented by th symbol NA
...
count / c(1,2) # the goal of R is to do element wise operations, so in this case it recycle the smaller vector to match the number of elements in the larger vector

1:10 > 7
states == "FL"

#filter vectors based on something 
#filtering uses the same syntax as sub-setting
...
# get the density values that are not from the staes of FL
